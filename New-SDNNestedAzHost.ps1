...
if ( ! $AzVirtualNetwork ) 
{
    ...
    $AzVNetSubnet = New-AzVirtualNetworkSubnetConfig -Name $SubnetName `
        -AddressPrefix $SubnetAddressPrefix

    $AzVirtualNetwork = New-AzVirtualNetwork -Name $VnetName -ResourceGroupName $ResourceGroupName `
...
else
{
    ...
    if ( ! $AzVNetSubnet )
    {
        ...
        $SubnetAddressPrefix = Read-Host "Prefix(ex:10.0.0.0/24)"
        $AzVNetSubnet = New-AzVirtualNetworkSubnetConfig -Name $SubnetName `
            -AddressPrefix $SubnetAddressPrefix -VirtualNetwork $AzVirtualNetwork
    }
...
if ( ! $AzNetworkSecurityGroup )
{
    ...
    $AzNetworkSecurityGroup = New-AzNetworkSecurityGroup -Name $NSGName `
        -ResourceGroupName $ResourceGroupName -Location $LocationName
}
else
{
...
{
    $AzNetworkSecurityGroup | Add-AzNetworkSecurityRuleConfig -Name "Rdp-Rule" `
        -Description "Allow WinRM" -Access "Allow" -Protocol "Tcp" -Direction "Inbound" `
...
{
    $AzNetworkSecurityGroup | Add-AzNetworkSecurityRuleConfig -Name "WinRM-Rule" `
        -Description "Allow WinRM" -Access "Allow" -Protocol "Tcp" -Direction "Inbound" `
        -Priority 150 -SourceAddressPrefix "Internet" -SourcePortRange "*" `
...
$AzPublicIpAddress = New-AzPublicIpAddress -Name $PublicIPAddressName `
    -DomainNameLabel $DNSNameLabel -ResourceGroupName $ResourceGroupName `
...
$AzNetworkInterface = New-AzNetworkInterface -Name $NICName -ResourceGroupName $ResourceGroupName `
    -Location $LocationName -SubnetId $AzVNetSubnet.Id -PublicIpAddressId $AzPublicIpAddress.Id `
...
$AzVM = Set-AzVMOperatingSystem -VM $AzVM -Windows -ComputerName $VMName `
    -Credential $Credential -ProvisionVMAgent -EnableAutoUpdate
$AzVM = Set-AzVMSourceImage -VM $AzVM -PublisherName 'MicrosoftWindowsServer' `
...
# Creating VM
New-AzVm -ResourceGroupName $ResourceGroupName -Location $LocationName -VM $AzVM `
    -LicenseType "Windows_Server" -Verbose    
if ( ! $AzVm )
{
...
else
{
    ...
    for ($i = 0; $i -lt $configdata.DiskNumber; $i++) 
    {
        ...
        $AzVM = Add-AzVMDataDisk -Name "$($VMName)_DataDisk$i" -Caching 'ReadWrite' -Lun $i `
            -ManagedDiskId $AzDisk.Id -CreateOption Attach -VM $AzVM
    ...
    Invoke-AzVMRunCommand -ResourceGroupName $ResourceGroupName -VMName $VMName `
        -ScriptPath $env:temp\injectedscript.ps1 -CommandId 'RunPowerShellScript'
    ...
    Invoke-Command $AzPublicIpAddress.DnsSettings.Fqdn -Credential $Credential {
        ...
        $virtualDisk = new-VirtualDisk -StoragePoolFriendlyName $StoragePool.FriendlyName `
                        -FriendlyName "VirtualDisk1" -ResiliencySettingName $VDiskResiliency `
                        -UseMaximumSize -NumberOfColumns $disks.Count
        ...
        $Partition = Get-VirtualDisk -FriendlyName $virtualDisk.FriendlyName | `
        ...
        $Credential = `
            New-Object System.Management.Automation.PSCredential ($AZFileUser, $AZFileSecurePassword) 
